...
3b. **price index** (決定的, `.devops/db/price-index.js`, 2026-08-20 運用者指示: 決定論的路は LLM 不使用) … llmpricing.dev の静的インデックス (`GET /api/models.json` + 各モデルページの埋め込み引用 JSON、CC-BY-4.0) を fetch し、公式 (lab) 価格と登録プロバイダーの引用を照合。フロンティア (または追跡中) モデルで公式価格の 90% 以下を提示する登録プロバイダーは DISCOUNTED offer として決定的に `verified` (通常価格 = 公式引用、現在価格 = プロバイダー引用)。`llmpricing_quotes` スナップショットで前日差分を取り、割引が消えた引用は `confirmed_removed` (discount ended)。カタログ所有プロバイダー (`api_catalog_url` あり) の引用は対象外。
3c. **aggregated index** (決定的, `.devops/db/aggregated-index.js`, 2026-08-22 運用者指示: 集約済みデータソースを優先し LLM の再巡回をやめる) … freellm.net のモデル表 (`GET /models/`: 401 モデル / 25 プロバイダー、free フラグ・permanent/quota tier・verified ・コンテキスト・スコア) と open-free-llm-api の README base URL 表 (30+ プロバイダー) を 2 fetch で取得し、crawl-facts `models[]` 形状に射影して discovery レーンと同じ reduction に流す (追加のみ・既知オファーは不変)。集約インデックスが free 検証済みの provider は provider_monitor の LLM セッションを spot check (visit 3) に縮小 (変更シグナルがある場合は従来の 12 visit)。
4. **known_refresh / 研究セッション ワーカー** (LLM, 並列, `GLOBAL_CONCURRENCY`=3 でスロットル, 2026-08-25 運用者指示: ブラウザ 6 並列はリソース競合で遅くするだけ) … known_refresh は公式ドキュメントから**生の事実のみ**を抽出し `schemas/crawl-facts.schema.json` で `json_output`。非カタログ provider は runId パリティで 2 日ローテーション (4 provider → 2 セッション/日, 小規模セット ≤2 はローテ無し)。研究セッション (spec 0008, spec 0007 のゴールクローラー 2 本は廃止) は news_scan (毎日 1 回) / vendor_deep_dive (シグナル駆動 + 7 日ローテーション, 1 日最大 2 本) / community_leads (毎日 1 回) / model_fanout (1 日最大 3 本) のみ。それぞれ `vendor-facts` / `leads` スキーマの `json_output`。web-search-plus 検索と browser 閲覧は各セッションの限定予算内で実施。**リトライは予算半減で再実行** (失敗セッションが 2 倍の時間を燃やさない)。研究セッションの失敗・産物は既知オファーを変更しない (spec 0007 AC-2 を継承)。
...
6. **benchmark bulk + benchmark_scout** + **benchmark reduce** (決定的優先, `.devops/db/benchmarks.js`) … Terminal-Bench 2.0/2.1 の公式 leaderboard を各1回取得し、全行を決定的に parse して current model と一括照合する。公式ページで明確に照合できない別名だけを `benchmark_scout` (LLM) に回す。`not_found` 検索は `NOT_FOUND_RESEARCH_TTL_DAYS` (14日) 後に自動で再調査する (公式 leaderboard やベンダー HF モデルカードはリリース後にスコアを公開することがあるため)。`found` はメタデータ変更または明示的な force 指定がない限り再調査しない。LLM の結果は**提案**であり、証拠検証 (テキストは本文確認、画像は HIGH confidence) を通ったものだけ不変の事実行になる。
7. **classifier / editor** (LLM) … classifier は `classification` の最終判定、editor は日本語本文のみを `editorial.json` に書く。データ状態は一切書かない。**入力は表示関連候補に事前フィルタ** (benchmark ≥50 / DISCOUNTED / caution / trial・campaign・conditional シグナルあり。`displayRelevantCandidate`, 2026-08-25 運用者指示: 全候補 200+ を分類するのは LLM 時間の無駄。落とされた候補は暫定分類のまま)。
...
- 掲載ポリシー (種別別, 2026-08-24 運用者指示): 完全無料/超激安枠は継続的なアクセスのみ。毎日/月ごとに補充される無料枠・明確な rate limit 付き継続提供の無料モデル・広告等で無料運営 → 掲載 (○)。初回クレジットだけ・Trial/Preview/Prototype エンドポイント → 掲載しない (×, `D_TRIAL_CREDIT`)。Hugging Face 等の一時キャンペーン → 別枠 `campaign_offers` (期間限定キャンペーン, 非空時のみ表示)。「NVIDIA NIM Free Endpoint」は原則掲載しない (×)。NVIDIA ファーストパーティ (model id が `nvidia/` namespace) の継続提供モデルのみ掲載可。機械的実施は `build/ranking-policy.js` (`CAMPAIGN_CLASSES` / `NON_PUBLISHABLE_CLASSES` / `nimFreeEndpointExclusion`) が唯一の真実源で、assembler / validator / builder が共有する。キャンペーン枠はランキングゲートの Terminal Bench 要件のみ免除 (endpoint 証拠・価格証拠・Gate 3・30B 閾値は同様に適用)。
