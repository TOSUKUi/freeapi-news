...
1. Read `build/provider-registry.json` (project root) first. For listed providers, use the registry `base_url` verbatim, fetch its `docs_url` to confirm the endpoint is unchanged, and set `endpoint_source` to that docs URL. If the fetched docs contradict the registry, emit a provider fact proposal; deterministic evidence audit stages the registry candidate and workers never write canonical registry files.
2. For providers NOT in the registry: fetch the official API docs (vendor domain only), copy the documented base URL and model ID format verbatim, emit a provider candidate with `added_from` set to the exact docs URL you fetched. Deterministic evidence audit stages the candidate registry entry; only after promotion may the offer be ranked. The validator re-fetches every citation and hard-fails when the page does not document the claimed base_url — an entry written from memory aborts the batch.
...
If benchmark data is found in any of these sources, extract the scores and attach them to the verification result as a `benchmarks` array (each entry: `{name, score, source_url, extraction_method, confidence}`). Do NOT write or read any state files: SQLite is the only operational state and the deterministic reducer owns acceptance. The benchmark scout task provides the exact model allowlist; a score for a model outside it is rejected. Do NOT mark a model as `insufficient_benchmark_data` unless all five source categories have been checked and yielded no usable scores.
